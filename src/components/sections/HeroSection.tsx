...
export function HeroSection({ mode = "experience", onModeChange }: Props) {
  ...
  return (
    <div className="min-h-[95dvh] flex flex-col pt-24 md:pt-0 pb-12 overflow-hidden">
      <div className="flex-1 px-6 md:px-12 lg:px-20 max-w-6xl w-full max-h-[80dvh] flex flex-col md:flex-row items-start md:items-center justify-start md:justify-between gap-6 pb-10">
        {/* Isometric Switcher */}
        ...
        {/* Text Content */}
        <div className="flex flex-col order-2 md:order-1 w-full md:max-w-sm lg:max-w-md text-left min-w-0">
          <MorphHeadline mode={activeMode} />
          <p
            className={`mt-4 max-w-xl text-lg md:text-xl leading-relaxed ${activeMode === "experience" ? "text-foreground opacity-90" : "text-zinc-400"}`}
          >
